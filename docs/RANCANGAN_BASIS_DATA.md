# RANCANGAN BASIS DATA SIMAGANG

Berkas ini merangkum struktur tabel beserta alasan di balik rancangannya.
Terakhir disamakan dengan skema yang berjalan pada 20 Agustus 2026.

Sumber kebenarannya ada di dua berkas yang **harus selalu diubah bersamaan**:

| Berkas | Untuk |
|---|---|
| `database/schema.sql` | MySQL |
| `database/schema.pgsql.sql` | PostgreSQL / Supabase |

Jalankan `php tools/banding-skema.php` untuk memastikan keduanya masih sama;
alat itu membandingkan basis data yang benar-benar berjalan, bukan berkasnya.

Perubahan struktur untuk instalasi MySQL yang sudah berjalan dicatat di
`database/UPGRADE.sql`. Pemasangan PostgreSQL baru tidak memerlukannya karena
`schema.pgsql.sql` sudah lengkap.

Tipe kolom di bawah ditulis dalam istilah MySQL. Padanannya di PostgreSQL:

| MySQL | PostgreSQL |
|---|---|
| `INT AUTO_INCREMENT` | `INT GENERATED BY DEFAULT AS IDENTITY` |
| `ENUM(...)` | `VARCHAR` + `CHECK (kolom IN (...))` |
...
- `reset_token_expires` (DATETIME, NULLABLE) — masa berlakunya dihitung di sisi basis data, bukan dengan jam PHP, agar tidak terpengaruh selisih zona waktu antara keduanya
...
dan wajib cocok dengan daftar nilai di atas — di MySQL berupa ENUM, di PostgreSQL
berupa CHECK constraint. `tests/TestJalurRusak.php` memeriksa kecocokannya.
