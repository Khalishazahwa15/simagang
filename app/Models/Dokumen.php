...
class Dokumen extends Model {
    ...
    public function findCurrentByPengajuanId($pengajuanId) {
        $stmt = $this->db->prepare("SELECT * FROM {$this->table} WHERE pengajuan_id = ? AND is_current = 1");
        $stmt->execute([$pengajuanId]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    public function findCurrentByPengajuanIdAndJenis($pengajuanId, $jenisDokumen) {
        $stmt = $this->db->prepare("SELECT * FROM {$this->table} WHERE pengajuan_id = ? AND jenis_dokumen = ? AND is_current = 1");
        $stmt->execute([$pengajuanId, $jenisDokumen]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }

    public function setNotCurrent($pengajuanId, $jenisDokumen) {
        $stmt = $this->db->prepare("UPDATE {$this->table} SET is_current = 0 WHERE pengajuan_id = ? AND jenis_dokumen = ?");
