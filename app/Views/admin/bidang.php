    ...
    <div style="background: var(--bg-card); border: 0.666667px solid var(--border); border-radius: 10px; overflow: hidden; margin-bottom: 24px;">
        <table style="width: 100%; border-collapse: collapse; min-width: 860px; text-align: left;">
            ...
            <tbody>
                <?php if (empty($divisi)): ?>
                <tr>
                    <td colspan="6" style="padding: 40px; text-align: center; color: var(--text-secondary); font-size: 13.5px;">Belum ada data divisi.</td>
                </tr>
                <?php else: ?>
                    <?php foreach ($divisi as $d): 
                        ...
                        $tersediaColor = $isFull ? '#9B2C2C' : 'var(--primary)';
                    ...
                    <tr style="border-bottom: 0.666667px solid var(--border); transition: background 0.15s; opacity: <?= $isAktif ? '1' : '0.6' ?>;" onmouseover="this.style.background='rgba(11,79,71,0.02)'" onmouseout="this.style.background='transparent'">
                        <td style="padding: 16px;">
                            <div style="font-weight: 600; font-size: 13.5px; line-height: 20px; color: var(--text-primary);">
                        ...
                        <td style="padding: 16px;">
                            <?php if ($isAktif): ?>
                                <span style="display: inline-flex; padding: 4px 10px; background: #E7F2EF; color: var(--primary); font-size: 11px; font-weight: 600; border-radius: 4px;">AKTIF</span>
                            <?php else: ?>
                                <span style="display: inline-flex; padding: 4px 10px; background: #F3F4F6; color: #4B5563; font-size: 11px; font-weight: 600; border-radius: 4px;">NONAKTIF</span>
                            <?php endif; ?>
                        </td>
                        <td style="padding: 16px; text-align: right;">
                            <div style="display: flex; align-items: center; justify-content: flex-end; gap: 6px;">
                                <button onclick="editDivisi(<?= htmlspecialchars(json_encode($d)) ?>)" style="display: flex; align-items: center; justify-content: center; width: 28px; height: 28px; background: #F7F4EC; border: 0.666667px solid var(--border); border-radius: 5px; cursor: pointer;" title="Edit Divisi">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" stroke-width="1.16667" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>
                                </button>
                                ...
                                <form action="<?= BASE_URL ?>/admin/bidang/toggle/<?= $d['id'] ?>" method="POST" style="margin: 0;">
                                    ...
                                    <button type="submit" onclick="return confirm('Apakah Anda yakin ingin <?= $isAktif ? 'menonaktifkan' : 'mengaktifkan' ?> divisi ini?')" style="display: flex; align-items: center; justify-content: center; width: 28px; height: 28px; background: <?= $isAktif ? '#FEE2E2' : '#E7F2EF' ?>; border: 0.666667px solid <?= $isAktif ? '#FECACA' : 'var(--primary-light)' ?>; border-radius: 5px; cursor: pointer;" title="<?= $isAktif ? 'Nonaktifkan' : 'Aktifkan' ?>">
                                        <?php if ($isAktif): ?>
...
<div id="modalTambahDivisi" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center;">
    <div style="background: #fff; width: 100%; max-width: 500px; border-radius: 12px; margin: 10vh auto; box-shadow: 0 4px 20px rgba(0,0,0,0.15);">
        <div style="padding: 20px 24px; border-bottom: 1px solid #eee; display: flex; justify-content: space-between; align-items: center;">
            ...
            <button onclick="document.getElementById('modalTambahDivisi').style.display='none'" style="background: none; border: none; cursor: pointer; color: #666;">
        ...
        <form action="<?= BASE_URL ?>/admin/bidang/store" method="POST">
            ...
            <div style="padding: 24px;">
                <div style="margin-bottom: 16px;">
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #333; margin-bottom: 8px;">Nama Divisi / Bidang <span style="color: red;">*</span></label>
                    <input type="text" name="nama_divisi" required style="width: 100%; padding: 10px 12px; border: 1px solid #ccc; border-radius: 6px; font-family: var(--font-body); font-size: 14px;">
                </div>
                <div style="margin-bottom: 16px;">
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #333; margin-bottom: 8px;">Deskripsi (Opsional)</label>
                    <textarea name="deskripsi" rows="3" style="width: 100%; padding: 10px 12px; border: 1px solid #ccc; border-radius: 6px; font-family: var(--font-body); font-size: 14px;"></textarea>
                </div>
                <div style="margin-bottom: 16px;">
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #333; margin-bottom: 8px;">Kapasitas Magang <span style="color: red;">*</span></label>
                    <input type="number" name="kapasitas" min="0" required value="0" style="width: 100%; padding: 10px 12px; border: 1px solid #ccc; border-radius: 6px; font-family: var(--font-body); font-size: 14px;">
                </div>
            </div>
            <div style="padding: 16px 24px; border-top: 1px solid #eee; display: flex; justify-content: flex-end; gap: 12px; background: #f9f9f9; border-bottom-left-radius: 12px; border-bottom-right-radius: 12px;">
                <button type="button" onclick="document.getElementById('modalTambahDivisi').style.display='none'" style="padding: 8px 16px; background: white; border: 1px solid #ccc; border-radius: 6px; cursor: pointer; font-weight: 600;">Batal</button>
...
<div id="modalEditDivisi" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center;">
    <div style="background: #fff; width: 100%; max-width: 500px; border-radius: 12px; margin: 10vh auto; box-shadow: 0 4px 20px rgba(0,0,0,0.15);">
        <div style="padding: 20px 24px; border-bottom: 1px solid #eee; display: flex; justify-content: space-between; align-items: center;">
            ...
            <button onclick="document.getElementById('modalEditDivisi').style.display='none'" style="background: none; border: none; cursor: pointer; color: #666;">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>
        </div>
        <form id="formEditDivisi" method="POST">
            ...
            <div style="padding: 24px;">
                <div style="margin-bottom: 16px;">
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #333; margin-bottom: 8px;">Nama Divisi / Bidang <span style="color: red;">*</span></label>
                    <input type="text" id="edit_nama_divisi" name="nama_divisi" required style="width: 100%; padding: 10px 12px; border: 1px solid #ccc; border-radius: 6px; font-family: var(--font-body); font-size: 14px;">
                </div>
                <div style="margin-bottom: 16px;">
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #333; margin-bottom: 8px;">Deskripsi (Opsional)</label>
                    <textarea id="edit_deskripsi" name="deskripsi" rows="3" style="width: 100%; padding: 10px 12px; border: 1px solid #ccc; border-radius: 6px; font-family: var(--font-body); font-size: 14px;"></textarea>
                </div>
                <div style="margin-bottom: 16px;">
                    <label style="display: block; font-size: 13px; font-weight: 600; color: #333; margin-bottom: 8px;">Kapasitas Magang <span style="color: red;">*</span></label>
                    <input type="number" id="edit_kapasitas" name="kapasitas" min="0" required style="width: 100%; padding: 10px 12px; border: 1px solid #ccc; border-radius: 6px; font-family: var(--font-body); font-size: 14px;">
                </div>
            </div>
            <div style="padding: 16px 24px; border-top: 1px solid #eee; display: flex; justify-content: flex-end; gap: 12px; background: #f9f9f9; border-bottom-left-radius: 12px; border-bottom-right-radius: 12px;">
                <button type="button" onclick="document.getElementById('modalEditDivisi').style.display='none'" style="padding: 8px 16px; background: white; border: 1px solid #ccc; border-radius: 6px; cursor: pointer; font-weight: 600;">Batal</button>
