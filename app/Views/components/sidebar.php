...
<aside class="admin-sidebar">
    ...
    <div class="sidebar-header">
        <a href="<?= BASE_URL ?>/" class="sidebar-brand">
            <img src="<?= aset('assets/img/bappeda-logo-kecil.png') ?>" alt="Logo Bappeda Provinsi Lampung" class="brand-logo">
            <div>
    ...
    <nav id="sidebar-navigation" class="sidebar-nav">
        ...
        <?php
        // Rotasi warna soft untuk ikon menu, supaya sidebar terasa lebih hidup
        // tapi tetap memakai palet soft yang sudah ada di tokens.css.
        $navColors = ['blue', 'amber', 'green'];
        $navColorIndex = 0;
        ?>
        <?php foreach ($menus as $menu): ?>
            ...
            <?php else: ?>
                <?php $navColor = $navColors[$navColorIndex % count($navColors)]; $navColorIndex++; ?>
                <a href="<?= $menu['url'] ?>" class="nav-item <?= $currentPage === $menu['id'] ? 'active' : '' ?>">
                    <span class="nav-icon nav-icon-<?= $navColor ?>"><?= $menu['icon'] ?></span>
                    <?= htmlspecialchars($menu['label']) ?>
                </a>
            <?php endif; ?>
        <?php endforeach; ?>
