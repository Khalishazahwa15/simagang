...
<header class="sticky top-0 z-[1000] border-b border-slate-200 bg-white">
    ...
    <div class="mx-auto flex min-h-[72px] w-full max-w-7xl items-center justify-between gap-6 px-6 sm:px-8 lg:px-12">
        ...
        <a href="<?= BASE_URL ?>/" class="flex items-center gap-3 no-underline">
            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-primary text-lg font-bold text-white shadow-sm">
                <span>S</span>
            </div>
            <div class="text-left">
        ...
        <nav class="hidden items-center gap-1 md:flex">
            <?php 
                ...
                foreach ($links as $id => $link) {
                    ...
                    echo "<a href='" . BASE_URL . $link['url'] . "' class='rounded-md px-3 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-primary {$activeClasses}'>{$link['label']}</a>";
                }
            ?>
        </nav>
        ...
        <div class="flex items-center gap-2">
            <a href="<?= BASE_URL ?>/login" class="hidden min-h-10 items-center rounded-lg border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 transition hover:border-primary hover:text-primary sm:inline-flex">Masuk</a>
            <a href="<?= BASE_URL ?>/register" class="hidden min-h-10 items-center rounded-lg bg-primary px-4 text-sm font-semibold text-white transition hover:bg-primary-dark sm:inline-flex">Daftar Sekarang</a>
    ...
    <div class="absolute left-0 right-0 hidden border-b border-slate-200 bg-white p-4 shadow-md md:hidden" id="mobile-nav">
        <?php foreach ($links as $id => $link): ?>
            ...
            <a href="<?= BASE_URL . $link['url'] ?>" class="mb-1 block rounded-md px-3 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-primary <?= $isActive ?>">
                <?= $link['label'] ?>
            </a>
        <?php endforeach; ?>
    </div>
